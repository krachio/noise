...
#[test]
fn compile_sine_dsp() {
    let dsp = soundman_faust::dsp::FaustDsp::from_code("sine", SINE_DSP, 48000, 256).unwrap();
...
#[test]
fn compile_gain_dsp() {
    let dsp = soundman_faust::dsp::FaustDsp::from_code("gain", GAIN_DSP, 48000, 256).unwrap();
...
fn sine_produces_audio() {
    let mut dsp =
        soundman_faust::dsp::FaustDsp::from_code("sine", SINE_DSP, 48000, 256).unwrap();
...
fn gain_processes_input() {
    let mut dsp =
        soundman_faust::dsp::FaustDsp::from_code("gain", GAIN_DSP, 48000, 256).unwrap();
...
fn faust_factory_creates_node_type_decl() {
    let factory = soundman_faust::factory::FaustFactory::new("sine", SINE_DSP);
...
fn invalid_code_returns_error() {
    let result = soundman_faust::dsp::FaustDsp::from_code("bad", INVALID_DSP, 48000, 256);
...
fn invalid_code_factory_probe_returns_error() {
    let factory = soundman_faust::factory::FaustFactory::new("bad", INVALID_DSP);
...
fn stereo_passthrough_channels() {
    let dsp =
        soundman_faust::dsp::FaustDsp::from_code("stereo", STEREO_DSP, 48000, 256).unwrap();
...
fn stereo_factory_probe_has_two_ports() {
    let factory = soundman_faust::factory::FaustFactory::new("stereo", STEREO_DSP);
