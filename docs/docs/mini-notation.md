...
|---|---|---|
| `x` or `X` | Hit (percussive trigger) | `krp.hit()` |
| `.` | Rest | `krp.rest()` |
| `~` | Rest (alternative) | `krp.rest()` |
| `-` | Rest (alternative) | `krp.rest()` |
| `C4`, `Cs4`, `C#4` | Note by pitch name | `krp.note("C4")` |
...
```python
krp.p("C4*2 E4 G4")       # C4 C4 E4 G4 (C4 played twice)
krp.p("x*4")              # equivalent to krp.hit() * 4
```
...
```python
kr.play("kick", krp.p("x . x . x . . x").swing(0.67))
kr.play("hat", krp.p("x x x x").over(2))
kr.play("bass", krp.p("C3 E3 G3 ~").every(4, lambda p: p.reverse()))
```
