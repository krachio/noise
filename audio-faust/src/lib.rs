//! FAUST LLVM JIT plugin for the [audio-engine] audio engine.
//!
//! Compiles [FAUST](https://faust.grame.fr) DSP code at runtime via LLVM JIT
//! and registers the resulting nodes in audio-engine's [`NodeRegistry`].
//!
//! # Quick start
//!
...
//! use audio_faust::register_faust_node;
//!
...
//! | [`factory`] | [`FaustFactory`] — audio-engine [`NodeFactory`] implementation |
//! | [`node`] | [`FaustNode`](node::FaustNode) — audio-engine [`DspNode`](audio_engine::graph::node::DspNode) adapter |
...
//!
//! [audio-engine]: https://docs.rs/audio-engine
//! [`NodeRegistry`]: audio_engine::registry::NodeRegistry
//! [`NodeFactory`]: audio_engine::registry::NodeFactory
...
/// Register a FAUST DSP program as a node type in the audio-engine registry.
///
