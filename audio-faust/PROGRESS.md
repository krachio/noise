...
FAUST LLVM JIT plugin for audio-engine with hot reload. 22 tests, clippy clean (all + pedantic + nursery).
...
- **Factory** (`factory.rs`): `FaustFactory` implements audio-engine's `NodeFactory` — probes ports and controls
- **Node** (`node.rs`): `FaustNode` adapts `FaustDsp` to audio-engine's `DspNode` trait
...
- Dependencies: audio-engine (path dep), log, notify
