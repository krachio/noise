...
- **soundman-core** (Rust): Audio engine — graph runtime with node reuse + return channel, gain smoothing, fan-in with NaN isolation, output clamping, registry versioning, pre-computed connection map, live control tracking, crossfade trigger forwarding — 116 tests.
...
- **Crossfade trigger forwarding**: SetParam now reaches both active AND retiring graphs during crossfade. Prevents ADSR decay on the old graph during blend.
- **Live control tracking**: EngineController records last-set value for each exposed control. Applied to new graphs after compilation — fresh nodes start with correct state.
- **SetBpm no-op guard**: Same-BPM set no longer nukes the event heap. Python tempo setter also skips redundant sends.
- **Division-by-zero guard**: crossfade_samples=0 no longer produces NaN in crossfade math.
