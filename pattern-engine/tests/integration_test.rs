...
use pattern_engine::ipc::protocol::ServerMessage;
use pattern_engine::output;
use pattern_engine::output::osc::OscSink;
...
fn ir_compile_and_query_integration() {
    ...
    let ir: pattern_engine::ir::IrNode = serde_json::from_str(json).unwrap();
    let pat = pattern_engine::ir::compile(&ir).unwrap();
